...
export default function Header() {
    ...
    return (
        <header className='z-[999] relative'>
            <motion.div 
                className={
                    'fixed top-0 left-1/2 -translate-x-1/2 h-[4.5rem] ' +
                    'w-full rounded-none border border-white border-opacity-40 ' +
                    'bg-white bg-opacity-80 shadow-lg shadow-black/[.03] ' +
                    'backdrop-blur-[.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[36rem] ' +
                    'sm:rounded-full dark:bg-gray-950 dark:border-border/40 dark:bg-opacity-75'
                }
            ...
            <nav className="flex fixed top-[.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
                ...
                {links.map((link) => {
                        ...
                        const href = isHomePage ? link.hash : `/${link.hash}`;
